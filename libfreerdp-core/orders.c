...
static void
process_windowing_window_information(rdpOrders * orders, STREAM s,
...
static void
process_windowing_notification_icon_information(rdpOrders * orders, STREAM s,
		uint32 fields_present_flags)
{
	uint32 window_id = 0;
...
static void
process_windowing_desktop_information(rdpOrders * orders, STREAM s,
		uint32 fields_present_flags)
{
...
static void
process_windowing_order(rdpOrders * orders, STREAM s)
{
	uint16 order_size;
	uint32 fields_present_flags;
	...
	{
		process_windowing_window_information(orders, s, fields_present_flags);
	}
	else if (fields_present_flags & WINDOW_ORDER_TYPE_NOTIFY)
	{
		process_windowing_notification_icon_information(orders, s,
				fields_present_flags);
	}
	else if (fields_present_flags & WINDOW_ORDER_TYPE_DESKTOP)
	{
		process_windowing_desktop_information(orders, s, fields_present_flags);
	}
	else
	{
		ui_unimpl(orders->rdp->inst,
...
process_alternate_secondary_order(rdpOrders * orders, STREAM s, uint8 order_flags)
{
	...
	switch (order_flags)
	{
		...
		case RDP_ORDER_ALTSEC_CREATE_OFFSCR_BITMAP:
			...
			break;
		case RDP_ORDER_ALTSEC_WINDOW:
			process_windowing_order(orders, s);
			break;
		default:
